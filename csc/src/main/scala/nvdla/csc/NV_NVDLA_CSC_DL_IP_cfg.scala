...
class NV_NVDLA_CSC_dl_reg2dp_if extends Bundle{
    val op_en = Output(Bool())
...
class NV_NVDLA_CSC_dl_subfield_d1_if(implicit val conf: nvdlaConfig) extends Bundle{
    val is_winograd_d1 = Output(UInt(22.W))
    val is_img_d1 = Output(UInt(34.W))
    val data_bank = Output(UInt(5.W))
    val datain_width = Output(UInt(14.W))
    val datain_width_cmp = Output(UInt(13.W))
    val datain_height_cmp = Output(UInt(13.W))
    val datain_channel_cmp = Output(UInt(11.W))
    val sub_h_total_g0 = Output(UInt(3.W))
    val sub_h_total_g1 = Output(UInt(3.W))
    val sub_h_total_g2 = Output(UInt(2.W)) 
    val sub_h_total_g3 = Output(UInt(3.W))
    val sub_h_total_g4 = Output(UInt(3.W))   
    val sub_h_total_g5 = Output(UInt(3.W))
    val sub_h_total_g6 = Output(UInt(3.W))
    val sub_h_total_g7 = Output(UInt(3.W)) 
    val sub_h_total_g8 = Output(UInt(3.W))  
    val sub_h_total_g9 = Output(UInt(3.W))  
    val sub_h_total_g10 = Output(UInt(3.W)) 
    val sub_h_total_g11 = Output(UInt(3.W))
    val sub_h_cmp_g0 = Output(UInt(3.W))
    val sub_h_cmp_g1 = Output(UInt(3.W))
    val conv_x_stride = Output(UInt(4.W))
    val conv_y_stride = Output(UInt(4.W))
    val pixel_x_stride_odd = Output(Bool())
    val data_batch = Output(UInt(6.W))
    val batch_cmp = Output(UInt(5.W))
    val pixel_x_init = Output(UInt(6.W))
    val pixel_x_init_offset = Output(UInt(7.W))
    val pixel_x_add = Output(UInt(7.W))
    val pixel_x_byte_stride = Output(UInt(7.W))
    val pixel_ch_stride = Output(UInt(12.W))
    val x_dilate = Output(UInt(6.W))
    val y_dilate = Output(UInt(6.W))
    val pad_value = Output(UInt(16.W))
    val entries = Output(UInt(conf.CSC_ENTRIES_NUM_WIDTH.W))
    val entries_batch = Output(UInt(conf.CSC_ENTRIES_NUM_WIDTH.W))
    val entries_cmp = Output(UInt(conf.CSC_ENTRIES_NUM_WIDTH.W))
    val h_offset_slice = Output(UInt(14.W))
    val rls_slices = Output(UInt(14.W))
    val slice_left = Output(UInt(14.W))
    val dataout_width_cmp = Output(UInt(13.W))
    val pra_truncate = Output(UInt(8.W))
    val pra_precision = Output(UInt(8.W))
}

class NV_NVDLA_CSC_dl_subfield_d2_if(implicit val conf: nvdlaConfig) extends Bundle{
    val h_bias_0_stride = Output(UInt(12.W))
    val h_bias_1_stride = Output(UInt(12.W))
    val h_bias_2_stride = Output(UInt(conf.CBUF_ADDR_WIDTH.W))
    val h_bias_3_stride = Output(UInt(conf.CBUF_ADDR_WIDTH.W))
    val rls_entries = Output(UInt(conf.CSC_ENTRIES_NUM_WIDTH.W))
}

class NV_NVDLA_CSC_dl_subfield_sg_done_if(implicit val conf: nvdlaConfig) extends Bundle{
    val last_slices = Output(UInt(14.W))
...
object NV_NVDLA_CSC_DL_IP_cfgDriver extends App {
  ...
  chisel3.Driver.execute(args, () => new NV_NVDLA_CSC_DL_IP_cfg())
}
