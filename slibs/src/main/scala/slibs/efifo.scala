...
class NV_NVDLA_CSC_SG_wt_fifo extends Module {
    ...
    val ram = Module(new NV_NVDLA_CSC_SG_wt_fifo_flopram_rwsa_4x8())
    ram.io.clk := io.clk
...
class NV_NVDLA_CSC_SG_wt_fifo_flopram_rwsa_4x8 extends Module {
    val io = IO(new Bundle {
        //clk
        val clk = Input(Clock())
...
withClock(io.clk){
    ...
    val di_d = Reg(UInt(8.W))
    when(io.iwe){
        di_d := io.di
    }

    withClock(io.clk_mgated){
        val ram_ff0 = Reg(UInt(8.W)) 
        val ram_ff1 = Reg(UInt(8.W))
        val ram_ff2 = Reg(UInt(8.W))
        val ram_ff3 = Reg(UInt(8.W))
        ...
        io.dout := MuxLookup(io.ra, "b0".asUInt(8.W), Seq(      
            0.U  -> ram_ff0,
            1.U  -> ram_ff1,
            2.U  -> ram_ff2,
            3.U  -> ram_ff3,
